...
$all = Intratum\Facturas\Environment::$db->get('serial');
...
<div class="py-5 px-10">
    ...
    <div class="w-full flex justify-between items-end mb-5 gap-2">   
        ...
        <a href="/configuracion/seriales/nuevo/" id="dropdownCust" class="flex bg-[#1A1917] rounded-md px-3 py-1 justify-center items-center gap-3 " >
    ...
    <div class="w-full ">
        ...
        <div class="mb-3 grid grid-cols-2 bg-[#ffffff] border-[1px] p-2 font-[700]">
            <h2>Serial</h2>
            <h2>Valor</h2>
            <h2></h2>
        </div>
        ...
        <?php
            foreach ($all as $i) {
        ?>
            <div data-url="/configuracion/seriales/ed?item=<?=Intratum\Facturas\Util::getUUIDByID2('se', $i["id2"]);?>" class="db_div cursor-pointer hover:bg-[#eee] bg-[#ffffff] border-[1px] mb-1 grid grid-cols-2 items-center p-2 rounded-xl">
			<?php if(!empty($i["serial_tag"])){ ?>
                <h2><?= $i["serial_tag"] ?></h2>
			<?php }else{ ?>
                <h2>(Sin prefijo)</h2>
			<?php } ?>
                            ...
                            <div class="">
                                <ul class="p-2 flex flex-col justify-center items-center">
                                    ...
                                    <li class="w-full flex gap-3 p-2 hover:bg-[#fafafafa] rounded-md text-[#000000]">
                                        ...
                                        <a href="/configuracion/seriales/ed?item=<?=Intratum\Facturas\Util::getUUIDByID2('se', $i["id2"]);?>" class="">Editar</a>    
                                    </li>

                                    <li class="w-full ">

                                        <a href="/configuracion/seriales/del?item=<?=Intratum\Facturas\Util::getUUIDByID2('se', $i["id2"]);?>" class="w-full flex gap-3 p-2 hover:bg-[#fafafafa] text-[#FE0000] rounded-md">
